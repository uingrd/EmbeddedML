...
void conv2d_s8(float* dout,     // 输出数据
    float* din,                 // 输入数据
    int din_hgt,                // 输入数据(矩阵)高度
    int din_wid,                // 输入数据(矩阵)宽度
    const signed char* ker,     // 卷积核
    const float* bias,          // 偏置
    const int* shape)           // 卷积核形状
{
    int din_size = din_hgt * din_wid;
    ...
    const float* din_sel;
    const signed char* ker_sel;
    const signed char* ker_elm;
    const float* din_elm;
    const float* din_elm0;
    float* dout_sel;
    float* dout_elm;
                    ...
                    ker_elm++)         // ker_elm指向元素内(kh, kw)位置元素
                {
                        ...
                        din_elm += din_wid)   // 指向数据滑动窗的下一行数据和ker_elm对应的数据位置
                    {
                        for (int w = 0; w < dout_wid; w++,
                            dout_elm++)
                        {
                            *dout_elm += din_elm[w] * ((float)(*ker_elm)) / ((float)128.0);
                        }
                    }
                }
            }
...
void fc_s8(float* dout,        // 输出数据
           float* din,         // 输入数据
           const signed char* weight,// 卷积核
           const float* bias,  // 偏置
           const int* shape)   // 卷积核形状
{
    // 数据尺寸
    ...
    for (int cout = 0; cout < num_cout; cout++)
    {
        ...
        d = din;
        for (int cin = 0; cin < num_cin; cin++, w++, d++)
            if (*w) dout[cout] += ((float)(*w)) * (*d) / ((float)128.0);
    }
}
...
int calc(float* din)
{
    ...
    fc(buf0, buf1, fc1_weight, fc1_bias, fc1_weight_shape);
    // x = F.relu(x)
    relu(buf0, buf0, 1024);
    // x = self.fc2(x)         # (N, 1024)->(N, 10)
    fc(buf1, buf0, fc2_weight, fc2_bias, fc2_weight_shape);
